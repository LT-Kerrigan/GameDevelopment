	...
	*/
	...
	for (unsigned int i = 0; i < numJoints; ++i) {
		...
		*/
		...
		skelJoint.transform = animQuat.ToMatrix();
		skelJoint.transform.SetPositionVector(animPos);
		...
		if (skelJoint.parent < 0) {	//Base Joint, so we're done
			skelJoint.transform = MD5FileData::conversionMatrix * skelJoint.transform;
		}
		else{
			...
			skelJoint.transform = parent.transform * skelJoint.transform;
		}
	}
}