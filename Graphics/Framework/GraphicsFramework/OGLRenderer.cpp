...
void OGLRenderer::SwapBuffers() {
	//If we've not manually called the debug drawing functions for this frame,
	//we'll do so here
...
/*
The following functions handle debug line drawing. Debug lines are pushed into vectors by the
DrawDebug[shape] functions, and output on screen by the DrawDebug[projection] functions. These
two functions clear their lists of lines after they have drawn them - this means that we can
call our debug draw functions whenever we want in the code, without worrying about whether
the renderer will clear the screen before they are drawn.
...
/*
Each of these draw debug functions does basically the same thing - it builds up
a number of lines and puts them into a vector - one for projection lines in
3D space, and one for ortho lines in 2D space.
*/
...
void	OGLRenderer::DrawDebugCircle(DebugDrawMode mode, const Vector3 &at, const float radius, const Vector3 &colour) {
	...
	const int stepCount = 18; //Modify this to make the circle more / less circular looking!
	...
	for (int i = 0; i < stepCount; ++i) {
		...
		target->AddLine(Vector3(startx, starty, at.z),
			Vector3(endx, endy, at.z), colour, colour);
	}
}
//Each set of lines is encapsulated in a DebugDrawData function.
//This just keeps track of pairs of vertices and colours
