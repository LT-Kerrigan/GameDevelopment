...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) {
	...
	for (int x = 0; x < LIGHTNUM; ++x) {
		for (int z = 0; z < LIGHTNUM; ++z) {
			...
			l.SetColour(Vector4(1.0f, g, b, 1.0f));
	...
	combineShader = new Shader(SHADERDIR"Test_15_CombineVertex.glsl", SHADERDIR"Test_15_CombineFrag.glsl");
	if (!combineShader->LinkProgram()) {
		return;
	}

	pointlightShader = new Shader(SHADERDIR"Test_15_PointLightVertex.glsl", SHADERDIR"Test_15_PointLightFrag.glsl");
...
void Renderer::FillBuffers() {
	...
	heightMap->Draw(); //drawInstaced maybe?
