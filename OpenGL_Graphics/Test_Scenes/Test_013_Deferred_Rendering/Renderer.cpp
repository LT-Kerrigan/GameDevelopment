...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) {
	...
	sceneShader = new Shader(SHADERDIR"BumpVertex.glsl", SHADERDIR"BufferFrag.glsl");
	if (!sceneShader->LinkProgram()) {
		return;
	}

	pointlightShader = new Shader(SHADERDIR"PointLightVertex.glsl", SHADERDIR"PointLightFrag.glsl");
	if (!pointlightShader->LinkProgram()){
		return;
	}

	combineShader = new Shader(SHADERDIR"CombineVertex.glsl", SHADERDIR"CombineFrag.glsl");
